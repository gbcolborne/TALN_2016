...
def normalize_chars(string):
    ...
    Get the normal form (C) of a unicode string, then replace
    characters that are not present in the Latin-1 encoding but that
    we want to keep (o+e ligatures, certains quotation
    marks). Finally, convert the string to Latin-1, discarding
    characters that are not in Latin-1, and convert back to unicode.
    """
...
if __name__ == "__main__":
    dsc = (u'Prétraitement du corpus PANACEA français : extraction du contenu textuel, '
           u'normalisation de caractères, segmentation et lemmatisation (facultative).')
    ...
    # Initialize tagger
    tagger = ttw.TreeTagger(TAGLANG='fr', TAGDIR='/usr/local/TreeTagger',
